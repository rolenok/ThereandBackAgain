	...
	for (int i = 0; i <NROWS; i++) {
		for (int j = 0; j <NCOLS; j++) {
			...
			for(int k = 1; k < n; k++) {
				...
				LU_Invert(multiply_matrices_integer_elements(p, Ap, NROWS, NCOLS), Unit_Permutation_Matrix, IA, NCOLS)); //rsold[idx(i,j,NROWS,NCOLS)] / (p[idx_transpose(i,j,NROWS,NCOLS)] * Ap[idx(i,j,NROWS,NCOLS)]);
